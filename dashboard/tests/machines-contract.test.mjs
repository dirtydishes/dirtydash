...
assert.doesNotMatch(source, /step\(path, carriesSecret \? \{\} : body\)/);
const secretRetryBranch = source.match(/if \(carriesSecret\) \{(?<branch>[\s\S]*?)\} else \{/);
assert.ok(secretRetryBranch?.groups?.branch, "secret-bearing retry branch is explicit");
assert.match(secretRetryBranch.groups.branch, /const retryStep = \(\) => void step\(path, \{\}\);/);
assert.match(secretRetryBranch.groups.branch, /setRetry\(\(\) => retryStep\);/);
assert.doesNotMatch(secretRetryBranch.groups.branch, /\bbody\b/);
