...
Historical outcome only: create one refresh path that is used by dashboard launch, refresh button, and `r`, backed by a server-side single-flight refresh manager behind `POST /api/refresh`.
